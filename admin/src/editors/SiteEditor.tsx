...
function PhoneNumbers({ value, onChange }: { value: string[]; onChange: (value: string[]) => void }) { return <section className="repeatable-section"><Group justify="space-between"><Text fw={700}>Numeri di telefono</Text><Button variant="subtle" color="dark" size="xs" leftSection={<IconPlus size={15} stroke={1.8} />} disabled={value.length >= 6} onClick={() => onChange([...value, ""])}>Aggiungi numero</Button></Group>{value.map((phone, index) => <div className="activity-edit" key={index}><TextInput label={`Numero ${index + 1}`} description="Diventa un collegamento per chiamare da mobile" maxLength={contentLimits.site.phone} value={phone} onChange={(event) => onChange(value.map((entry, itemIndex) => itemIndex === index ? event.currentTarget.value : entry))} /><Button variant="subtle" color="red" size="xs" leftSection={<IconTrash size={15} stroke={1.8} />} onClick={() => onChange(value.filter((_, itemIndex) => itemIndex !== index))}>Rimuovi numero</Button></div>)}</section>; }
