...
export function App() {
  ...
  return <AppShell navbar={{ width: 258, breakpoint: "sm", collapsed: { mobile: !mobileNavOpened } }} header={{ height: 58, collapsed: !isMobile }} padding={0} className="admin-app-shell">
    ...
    <AppShell.Main className="admin-main"><main className="admin-workspace">
      ...
      {section === "team" && editing && <TeamEditor key={editingId} entity={currentTeam} teamCount={activeTeam.length} siteSettings={siteContent ?? defaultSiteSettings} onBack={closeEditor} onSaved={saved} onPublish={publish} onArchive={archive} onDirtyChange={setDirty} />}
...
function TeamEditor({ entity, teamCount, siteSettings, onBack, onSaved, onPublish, onArchive, onDirtyChange }: { entity?: ContentEntity<TeamMemberContent>; teamCount: number; siteSettings: SiteSettingsContent; onBack: () => void; onSaved: (entity: ContentEntity, message?: string) => Promise<void>; onPublish: (resource: AdminResource, id: string) => Promise<void>; onArchive: (resource: AdminResource, id: string) => Promise<void>; onDirtyChange?: (dirty: boolean) => void }) {
...
function SiteIdentity({ value, update }: { value: SiteSettingsContent; update: (recipe: (draft: SiteSettingsContent) => void) => void }) {
  ...
  return <div className="home-sections">
    <section className="form-section">
      <SectionHeading title="Nome e logo" hint="Compaiono nell’intestazione di ogni pagina e in cima al piè di pagina." shape="hero" />
      <div className="form-grid">
      ...
      <MediaPicker label="Logo" previewTone="dark" previewFit="contain" previewRatio={16 / 6} value={{ url: value.identity.logoLight ?? "", alt: `${value.identity.organizationName} logo` }} onChange={(next) => update((draft) => { draft.identity.logoLight = next.url || undefined; })} />
    ...
    <section className="form-section">
      <SectionHeading title="Recapiti nel piè di pagina" hint="Chiudono la home, sotto il blocco «04 — Contatti»: forma giuridica, indirizzo, contatti e social." shape="notes" />
      <div className="form-grid">
        {campo("legalForm", "Forma giuridica", { descrizione: "Prima riga del piè di pagina", max: contentLimits.site.legalForm })}
        ...
        {campo("phone", "Telefono", { descrizione: "Facoltativo: diventa un collegamento per chiamare", max: contentLimits.site.phone, opzionale: true })}
...
function TeamPreview({ member, site, height }: { member: TeamMemberContent; site: SiteSettingsContent; height?: number }) {
  return <PreviewFrame height={height}><PeopleHero teamMembers={[teamToLegacy(member)]} site={site} /></PreviewFrame>;
}
